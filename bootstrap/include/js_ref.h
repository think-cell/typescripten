...
protected:
	...
	T _getProperty(Name&& name) noexcept {
		...
		static_assert(IsJsInteropable<T>::value);
		return m_emval[js_string(std::forward<Name>(name))].template as<T>();
	}
	...
	void _setProperty(Name&& name, T const& value) noexcept {
		static_assert(std::is_constructible<js_string, Name&&>::value);
		static_assert(IsJsInteropable<T>::value);
		m_emval.set(js_string(std::forward<Name>(name)), value);
	}
...
template<typename T> typename T::_tcjs_definitions base_detector(T*);
